...
import { calculateYearSummary, getYearEvents, calculateYearGrowth } from '../utils/yearSummaryUtils';
...
const YearSummary: React.FC<YearSummaryProps> = ({ year, events, isOpen, onClose, allEvents }) => {
  const summary = calculateYearSummary(allEvents, year);
  const yearEvents = getYearEvents(allEvents, year);
  const growth = calculateYearGrowth(allEvents, year);

  const calculateNewVsLeft = () => {
    const newEmployees = yearEvents.filter(e => e.type === EventType.NEW_EMPLOYEE).length;
    const leftEmployees = yearEvents.filter(e => 
      e.type === EventType.EMPLOYEE_LEFT || 
      e.type === EventType.EMPLOYEE_TERMINATED
    ).length;
    
    return { 
      newEmployees, 
      leftEmployees,
      isPositive: newEmployees >= leftEmployees 
    };
  };

  const calculateRetention = () => {
    const startYearEvents = yearEvents.filter(event => 
      event.type === EventType.NEW_EMPLOYEE || 
      event.type === EventType.EMPLOYEE_LEFT || 
      event.type === EventType.EMPLOYEE_TERMINATED
    );

    const totalEmployees = startYearEvents.filter(e => e.type === EventType.NEW_EMPLOYEE).length;
    const leftEmployees = startYearEvents.filter(e => 
      e.type === EventType.EMPLOYEE_LEFT || 
      e.type === EventType.EMPLOYEE_TERMINATED
    ).length;

    if (totalEmployees === 0) return 100;
    return Math.round(((totalEmployees - leftEmployees) / totalEmployees) * 100);
  };

  const renderEventList = (eventType: EventType) => {
    const filteredEvents = yearEvents.filter(event => event.type === eventType);
          ...
          <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white p-6 shadow-xl transition-all">
            ...
            <div className="grid grid-cols-3 gap-6 mb-8">
              <div className="bg-green-50 rounded-lg p-4">
                <div className="text-sm text-green-600 mb-1">Team Growth</div>
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <div className="text-sm text-blue-600 mb-1">Retention Rate</div>
                <div className="flex items-center gap-2">
              ...
              <div className={`rounded-lg p-4 ${
                calculateNewVsLeft().isPositive ? 'bg-green-50' : 'bg-red-50'
              }`}>
                <div className={`text-sm mb-1 ${
                  calculateNewVsLeft().isPositive ? 'text-green-600' : 'text-red-600'
                }`}>
                  New vs Departures
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-2xl font-bold ${
                    calculateNewVsLeft().isPositive ? 'text-green-700' : 'text-red-700'
                  }`}>
                    {calculateNewVsLeft().newEmployees} / {calculateNewVsLeft().leftEmployees}
                  </span>
                </div>
              </div>
            </div>
